...
import restapi.util.dataFake.model.resources.CourseRespDf;
...
@DisplayName("restapi :: controller :: CourseController")
public class CourseControllerTest {
    ...
    @InjectMocks
    private CourseController controler;

    @Autowired
    private CourseRespDf respDf;

    @Autowired
    private CourseReqDf reqDf;

    private List<CourseResp> list;
    private CourseResp obj;
    ...
    public void getAll() {
        try {
            ...
            ResponseEntity<List<CourseResp>> responseList = controler.getAll();
    ...
    public void getById() {
        try {
            ...
            Long id = 1L;
            ResponseEntity<CourseResp> response = controler.getById(id);
    ...
    public void create() {
        try {
            Mockito.when(serviceMock.create(Mockito.any(CourseReq.class))).thenReturn(obj);

            ResponseEntity<CourseResp> response = controler.create(reqDf.getData());
    ...
    public void update() {
        try {
            Mockito.when(serviceMock.update(Mockito.anyLong(), Mockito.any(CourseReq.class))).thenReturn(obj);

            Long id = 1L;
            ResponseEntity<CourseResp> response = controler.update(id, reqDf.getData());
